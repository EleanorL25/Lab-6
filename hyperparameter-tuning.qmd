...
```{r}
predictions<-ggplot(full_pred, aes(x=.pred, y=logQmean))+
  geom_point()+
  geom_smooth(method = 'lm')+
  labs(x="predictions", y="LogQMean", title="Random Forest Model Predictions")
predictions
```
...
```{r}
residuals<-ggplot(full_pred_mutated, aes(x=residuals, y=logQmean))+
  geom_point()+
  geom_smooth(method='lm')+
  labs(x="residuals", y="LogQMean", title="Residuals for Random Forest Model")
residuals
```
